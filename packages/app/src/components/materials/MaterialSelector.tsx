/**
 * MaterialSelector - Full-featured material selector with categories,
 * search, recent materials, favorites, live preview, and custom editor.
 */
...
export function MaterialSelector({
  ...
  // Clear preview when unmounting or partId changes
  useEffect(() => {
    return () => setPreviewMaterial(null);
  }, [partId, setPreviewMaterial]);
  ...
  const handleSelect = useCallback(
    ...
    [partId, setPartMaterial, addRecentMaterial, setPreviewMaterial]
  );

  const handlePreview = useCallback(
    (material: MaterialPreset | null) => {
      if (material) {
        setPreviewMaterial({ partId, materialKey: material.key });
      } else {
        setPreviewMaterial(null);
      }
    },
  ...
  const handleCustomMaterialSave = useCallback(
    (mat: Omit<MaterialPreset, "key">) => {
      ...
      // Apply to part
      setPartMaterial(partId, key);
      addRecentMaterial(key);
    },
    [partId, customMaterialCount, setPartMaterial, addRecentMaterial]
  );
